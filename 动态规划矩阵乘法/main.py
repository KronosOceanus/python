...
def MatrixMultiplication(inp):
    ...
    for r in range(1, inp):
        for i in range(inp - r):
            j = i + r       # i，j 分别为正要计算的最优乘法顺序的两个矩阵
            m[i][j] = m[i + 1][j] + matrix[i][0] * matrix[i][1] * matrix[j][1]      # 递推公式（少了左边一半）
            s[i][j] = i + 1     # 默认状态，直接乘下一个矩阵（输出矩阵相乘顺序要用）
            for k in range(i + 1, j):
                judge = m[i][k] + m[k + 1][j] + matrix[i][0] * matrix[k][1] * matrix[j][1]      # 乘法次数递推公式
                if judge < m[i][j]:
                    m[i][j] = judge
                    s[i][j] = k + 1


def printmatrix(left, right):       # 输出乘法顺序
    if left == right:
